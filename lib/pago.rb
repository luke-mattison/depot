...
class Pago
  def self.make_payment(order_id:, payment_method:, payment_details:)
    ...
    sleep 3 unless Rails.env.test? # it is for simulate some delay in the third party service.

    Rails.logger.info "Done Processing Payment"
    OpenStruct.new(succeeded?: true) # it return a generic object that respond to "succeeded?"
  end
end
